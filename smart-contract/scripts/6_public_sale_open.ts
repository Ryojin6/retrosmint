...
async function main() {
  ...
  const publicSalePrice = utils.parseEther(CollectionConfig.publicSale.cost2.toString());
  if (!await (await contract.cost2()).eq(publicSalePrice)) {
    console.log(`Updating the token price to ${CollectionConfig.publicSale.cost2} ${CollectionConfig.mainnet.symbol}...`);
