...
async function main() {
  ...
  const preSalePrice = utils.parseEther(CollectionConfig.preSale.cost2.toString());
  if (!await (await contract.cost2()).eq(preSalePrice)) {
    console.log(`Updating the token price to ${CollectionConfig.preSale.cost2} ${CollectionConfig.mainnet.symbol}...`);
