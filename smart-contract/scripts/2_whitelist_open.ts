...
async function main() {
  ...
  const whitelistPrice = utils.parseEther(CollectionConfig.whitelistSale.cost1.toString());
  if (!await (await contract.cost1()).eq(whitelistPrice)) {
    console.log(`Updating the token price to ${CollectionConfig.whitelistSale.cost1} ${CollectionConfig.mainnet.symbol}...`);
